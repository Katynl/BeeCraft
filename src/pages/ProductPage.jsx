...
const getOptimizedImage = (url, width = 800, height = 1000) => {
  if (!url?.includes("cloudinary.com")) return url;
  return `${url}?w=${width}&h=${height}&fit=crop&q=80&f=auto`;
};

// ========== Мемоизированная карточка похожего товара ==========
const SimilarProductCard = React.memo(({ product }) => {
  const optimizedImage = getOptimizedImage(product.image_url, 400, 533);

  return (
    <Link
      to={`/catalog/${product.slug}`}
      aria-label={`Открыть похожий товар: ${product.name}`}
      className={`group transition-all duration-500 hover:-translate-y-1 ${focusClass}`}
    >
      <div className="aspect-[3/4] overflow-hidden bg-stone-100">
        <img
          src={optimizedImage}
          alt={product.name || "Похожий товар Bee Craft"}
...
const ProductPage = React.memo(() => {
  ...
  // Загрузка данных с отменой запроса
  useEffect(() => {
    let isMounted = true;
  ...
  // Мемоизация рендера похожих товаров
  const similarProductsMarkup = useMemo(() => {
  ...
  const mainImageOptimized = getOptimizedImage(product.image_url, 800, 1000);
